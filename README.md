...
Drop files onto the message composer or use Attach files. The original files stay with the current project; uploads preserve the draft and saved source. Multiple files queue with at most two active transfers, real progress, cancellation, and download links. Limits are 250 MiB per file, 1 GiB and 20 attachments per project. Zip a folder before dropping it. The dashboard static-source importer remains separate.

Select attachments and send an instruction. Text and ZIP source excerpts use a bounded 200 KB context budget. OpenAI, Anthropic, and Google adapters support native PNG/JPEG/WebP and PDF input, with a combined 20 MiB selected-media limit. GIF and other binary files are stored for download but have no content reader in this increment; the interface labels this limitation. Media exceeding the reading limit stays stored and is omitted from AI input. Provider model/size limits may still reject a request. No file is executed during upload or inspection, and base64 media bytes are never embedded as ordinary prompt text.

`ATTACHMENT_STORAGE_DIR` selects storage (default `data/attachments`). Back up that directory together with PostgreSQL. Compose mounts `attachment-data` for the originals. Downloads use attachment disposition and an inert content type. ZIP structure and bounded source excerpts are validated separately. Common credential/dependency paths are excluded from text context. Uploading alone makes no model request; selected content is sent to the chosen provider when an instruction is submitted.

Native image/PDF retrieval is restricted to the authenticated build worker, a live lease, and that build's selected attachments. Build metadata records the chosen context. Full archive migration, server runtimes, and parsing every binary document format remain outside this increment.
